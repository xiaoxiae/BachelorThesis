...
over the observed features, and an appropriate cost function. One such cost function is the nonlinear least squares method, formulated as 
...
One of the algorithms that can be used is Poisson surface reconstruction \cite{kazhdan2006poisson}.
...
It does this by first computing a piecewise indicator function $\chi$ that is $0$ outside of the model and $1$ inside.

Since the gradient of this indicator function (when convoluted with a suitable smoothing filter) is a vector field, whose values near the surface are the point normals, the problem can be formulated as a minimization problem in terms of the difference between observed normals as a vector field (approximated by a discrete sum, since we only have a set of points and their normals), and the indicator function gradient:

...
After the features have been determined, the next step is to simultaneously calculate their position in the 3D space and also the positions of the cameras.
This is referred to as the bundle adjustment problem \cite{schneider19913}, the name referring to bundles of rays from the predicted positions of the points going into the cameras.

More formally, we can imagine the scene being modeled by a vector of $n$ 3D points $\mathbf{X}_i$, taken from $m$ cameras with parameters (position, focal length, etc.) given by the vector $\mathbf{P}_j$.

...
\begin{itemize}
	\item It created the highest quality models out of all of the tested programs.
...
The quality could be further improved by using laser scanning, which handles glossy surfaces better than using only images.
...
After all models have been generated, a metadata file is created for all successfully generated ones.
The specification is as follows:

\begin{minted}{yaml}
<the first 12 characters of the sha256sum of model data>:
  color: [<name>, <hex value>]
  type: <the type of hold>
...
The first 12 characters of the hashsum were chosen for readability, and because tests for collisions on random data showed that tens of thousands of holds would be required for a reasonable chance of collision.
...
The editor code is cross-platform and can be build on Mac, Windows and Linux, with the newest builds being freely available on the project GitHub page.
...
\begin{minted}{yaml}
# Cled version (for future compatibility reasons)
Version: 1.0
...
# a dictionary of hold instances
Holds:
...
This can be done either from the \verb|Lighting| section of the toolbar, or by using \verb|F| to toggle the player lighting and \verb|Ctrl+F| to place it.

TODO: image before lighting scene and after lighting scene
